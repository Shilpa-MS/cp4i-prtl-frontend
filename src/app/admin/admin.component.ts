...
export class AdminComponent implements OnInit {
  ...
  approvedsendmail(approvemail) {
    let mailbody = {
      ...
      text:
        "Registration is approved by Admin...Plz check on http://jumpstart-portal-test.mybluemix.net/ ",
      html:
  ...
  getUser() {
    ...
    this.menuService.fetchAllUserData().subscribe((data: any) => {
      ...
      this.userdata.forEach((userFulldata) => {
        ...
        if (
          userFulldata.user_status == "approved" ||
          userFulldata.user_status == "rejected"
        ) {
  ...
  approve(userapproveData) {
    ...
    this.menuService
      .UpdateUserStatus(userapproveData)
      .subscribe((data: any) => {
        //console.log("userdata approve data is admin",this.userdata,userapproveData);
        this.getUser();
        this.enable_flag = true;
      });
    if (this.enable_flag == true) this.approvedsendmail(userapproveData.email);
  }
  ...
  reject(userrejectData) {
    ...
    if (this.enable_flag == true) this.rejectedsendmail(userrejectData.email);
  }
}
