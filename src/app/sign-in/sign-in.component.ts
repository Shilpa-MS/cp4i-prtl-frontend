...
export class SignInComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.registrationForm = this.fb.group({
      ...
      email: [
        null,
        Validators.compose([Validators.required, emailValidator.strong]),
      ],
  ...
  login(formvalue) {
    ...
    this.menuService.UserAuthenticate(formvalue).subscribe(
      (data: any) => {
        if (data.status == "200") {
          ...
          if (
            (this.loginData.user_status == "approved" &&
              this.loginData.access[0].jumpstart == true) ||
  ...
  registration(regFormValue) {
    this.menuService
      .JumpstartfetchUserData(regFormValue.email)
      .subscribe((data: any) => {
  ...
  checkans() {
    ...
    this.menuService.compareSecurityQuesforuser(payload).subscribe(
      (data: any) => {
        if (data.status == "200") {
          console.log("inside if");
          //alert(data.message);
      ...
      (error) => {
        console.log("errro", error);
        if (error.status == "404") alert("User does not exist!");
        if (error.status == "401") alert("answer incorrect!");
        if (error.status == "500") alert("server error!");
        if (error.status == "502") alert("secret answer should not be null!");
      }
    );
  }
